...
final class PeonyParser
{
    ...
    public function parse(string $pdfPath): array
    {
        ...
        // Fallback: lineprinter + kerning normalization
        $text = $this->runPdftotext($pdfPath, '-lineprinter');
        if ($text === null) {
            throw new RuntimeException('pdftotext unavailable or failed on: ' . basename($pdfPath));
        }
        $this->lastBackend = 'lineprinter';
        $warnings[] = 'Usando backend lineprinter (poppler no detectado) — kerning irregular puede reducir precisión.';
